...
import cm4mmupdater.customTypes.FullUpdateInfo;
import cm4mmupdater.customTypes.ThemeInfo;
import cm4mmupdater.customTypes.ThemeList;
...
public class UpdateCheckService extends Service {
    ...
    // UGLY HACK - Invoke checkForNewUpdates() if startService was called from automatic update, else let the interface do the job
    // TODO Make this one pretty
    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {
    	if (intent.toString().equals("Intent { act=cm4mmupdater.interfaces.IUpdateCheckService }")){
    		if (showDebugOutput) Log.d(TAG, "Call came from UI. Let it do its job alone");   		
    		isAutoUpdate = false;
    	}
    	else{
    		if (showDebugOutput) Log.d(TAG, "Call came from automatic update check. Invoking checkForNewUpdates()");
    		Log.i(TAG, "Automatically checking for updates (scheduled)");
    		isAutoUpdate = true;
    		checkForNewUpdates();
    	}
		return startId;
    ...
    private void checkForNewUpdates() {
        ...
        if (updateCountRoms == 0 && updateCountThemes == 0 && updateCountIncrementalRoms == 0) {
        	Log.i(TAG, "No new updates found");
            if (!isAutoUpdate) ToastHandler.sendMessage(ToastHandler.obtainMessage(0, R.string.no_updates_found, 0));
            FinishUpdateCheck();
        } else {
        	Log.i(TAG, "Found "+updateCount+" new updates");
    ...
    private void FinishUpdateCheck() {
        ...
        if (isAutoUpdate){
        	if (showDebugOutput) Log.d(TAG, "AutoUpdate done. Stopping UpdateCheckService");
        	this.stopSelf();
        }
    }
}